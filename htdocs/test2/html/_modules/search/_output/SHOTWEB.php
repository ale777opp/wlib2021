...
foreach ($response0 as $key => $value)
{
	...
	if($res !== false)
	{
			...
			for($i=0; $i<$len; $i++)
			{
				if($arr[$i]!="")
				{
					...
					else
					{
						...
						{
							if((isset($result->response_0->_renew))&&($start == 0)&&(($newsoutform=="")||($newsoutform=="SHOTWEB")))
							{
		...
		{
			$tabs.='<span title="rusmarc" class="add1" onmousedown="seeRusmarc(this,\''.$theid.'\',\''.$count.'\',\''.$realdb.'\')">RUSMARC</span>';
