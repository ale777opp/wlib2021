...
$globaloutput.= <<<HTML
				...
				<div class="td vtop content pr20x pb20x pl20x brl1c h100">
					...
					<!--<div class="newscontrols">
						<span onmousedown="searchPublications(21,'all')">Все</span>
						<span onmousedown="searchPublications(21,'archiv')">Архив</span>
					</div>-->
					...
					for($i=1; $i<$len; $i++)
					{
						if($arr[$i]!="")
						{
							...
							else
							{
								...
								if($res3 !== false)
								{
									$output.='<p><a target="_blank" href="'.substr($arr[$i], $res3+5).'">Полный текст</a></p>';
								}
								else
									$output.='<p>'.parseBB($arr[$i]).'</p>';
							}
						}
					}
					$output.='</div>';
